	...
	providers: [
		{
			...
			token: {
				url: `${WCA_URL}/oauth/token`,
				async request(context) {
					const provider = context.provider
					const query = `${WCA_URL}/oauth/token?client_id=${provider.clientId}&client_secret=${provider.clientSecret}&redirect_uri=${provider.callbackUrl}&code=${context.params.code}&grant_type=authorization_code`
					const res = await fetch(query, { method: 'POST' })
					const json = await res.json()
					return { tokens: json }
				},
			},
			userinfo: `${WCA_URL}/api/v0/me`,
			profile(profile) {
				return {
					...profile.me,
